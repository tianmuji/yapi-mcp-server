...
export function formatListMenu(data: any[]): string {
  if (!Array.isArray(data) || data.length === 0) return "No interfaces found.";
...
export function formatInterfaceDetail(data: any): string {
  ...
  if (data.markdown) lines.push(`**Docs:**\n${data.markdown}`);
...
/** Format project info */
export function formatProjectInfo(data: any): string {
