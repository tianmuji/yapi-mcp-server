...
const yapi_client_js_1 = require("./yapi-client.js");
...
server.tool("list_apis", "List all API interfaces grouped by category for a YApi project", {
    ...
    return { content: [{ type: "text", text: (0, formatters_js_1.formatListMenu)(res.data) }] };
});
...
server.tool("search_api", "Search API interfaces by path or title keyword within a YApi project", {
    ...
    for (const cat of res.data || []) {
        for (const item of cat.list || []) {
            if ((item.path && item.path.toLowerCase().includes(kw)) ||
    ...
    const lines = matches.map((m) => `[${m.method?.toUpperCase() || "?"}] ${m.path} — ${m.title} (id: ${m._id}, category: ${m._catName})`);
    return {
...
server.tool("get_project_info", "Get basic information about a YApi project", {
    ...
    return { content: [{ type: "text", text: (0, formatters_js_1.formatProjectInfo)(res.data) }] };
});
...
server.tool("import_api_docs", "Batch import full API documentation for a YApi project. Returns complete details (params, request body, response schema) for all matching APIs. Supports filtering by category or keyword. Use this to get comprehensive API docs in one call.", {
    project_id: v3_1.z.string().describe("YApi project ID"),
    cat_id: v3_1.z.string().optional().describe("Filter by category ID (from list_apis). If omitted, imports all categories."),
    ...
    for (const cat of menuRes.data || []) {
        if (cat_id && String(cat._id) !== cat_id)
            continue;
        for (const item of cat.list || []) {
            if (kw) {
    ...
    for (let i = 0; i < toFetch.length; i += batchSize) {
        ...
        for (const { item, detail } of results) {
            ...
            docs.push((0, formatters_js_1.formatInterfaceDetail)(detail));
            docs.push("\n---\n");
        }
    }
...
// Tool 9: update_api
server.tool("update_api", "Update an existing API interface. Only provided fields will be modified, others remain unchanged.", {
    interface_id: v3_1.z.string().describe("Interface ID to update (from list_apis or search_api)"),
    title: v3_1.z.string().optional().describe("New title"),
    path: v3_1.z.string().optional().describe("New API path"),
    ...
    try {
        ...
        if (method !== undefined)
            changes.push(`method → ${method.toUpperCase()}`);
        if (cat_id !== undefined)
            changes.push(`category → ${cat_id}`);
        if (desc !== undefined)
